...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (!validateEmail(userCred)) {
      formik.setValues({ ...formik.values, username: userCred, email: "" });
    } else {
      formik.setValues({ ...formik.values, email: userCred, username: "" });
    }
  }, [userCred]);

  return (
    <div className="h-screen w-screen bg-main flex flex-col justify-center items-center">
      <div className="flex text-white flex-col gap-[2rem] items-center py-5 h-2/3 w-auto md:w-1/3">
        <div className="relative py-3 min-w-[300px] md:w-5/6 sm:mx-auto">
          <div className="relative px-4 py-10 bg-card mx-8 md:mx-0 shadow rounded-3xl sm:p-10">
            <div className="max-w-md mx-auto">
              <div className="flex items-center space-x-5 justify-center font-bold">
                Login
              </div>
              <form
                action="
